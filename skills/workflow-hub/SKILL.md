...
本技能整合《01-environment》《02-git-workflow》《03-code-quality》《04-testing》《05-build-deploy》的关键要求，为所有任务提供统一的操作顺序。
...
- **Git 缺少 cache**：`git remote get-url cache` 检查路径，不存在则 `git remote add cache <path>` 并确保裸仓库存在。
...
- **输出格式**：
  ...
  === 环境检测 ===
  项目类型: Vue 3 (pnpm)
  Git 环境: 工作区 /home/xxx/project-wt1，分支 main，状态 clean
...
1. **环境识别**  
   - 使用上面脚本定位仓库角色。若为工作区，记录主项目与 cache 路径；若 remote 缺失，立即汇报。  
   - 记录当前分支与工作区脏/洁净状态。
...
2. **意图拆解**  
   - 结合触发词将需求映射到子技能：  
     | 用户意图 | 需调用技能 |
     | --- | --- |
     | “保存进度”“提交到工作区”“备份代码” | `workflow-git`（工作区→cache） |
     | “发布代码”“合并主项目”“推送服务器” | `workflow-git`（完整发布） |
     | “提交前检查”“质量检查”“跑测试” | `workflow-quality-release`（质量+测试） |
     | “构建”“部署”“上线” | `workflow-quality-release`（构建/部署章节） |

3. **流程执行**  
   - **Git 操作**：严格遵循 `workflow-git` 的识别、cache 推送、主项目合并与发布策略，禁止跳过任一步。  
...
- Git 相关动作必须先加载 `../workflow-git/SKILL.md`，严格执行三层流程与危险操作约束。
...
| 触发词 | 答复要点 |
| --- | --- |
| “提交代码” | 询问是保存进度（工作区→cache）还是正式发布（cache→主项目→远端）。 |
| “保存进度” | 解释将推送到 cache，不影响主项目。 |
| “发布/上线/推送服务器” | 列出完整步骤（push cache → 切主项目 → fetch/merge → push origin → 回同步）。 |
| “质量/测试” | 输出计划（格式化→Lint→类型→测试→安全），逐项执行并给结果。 |
| “构建/部署” | 先确认环境与变量，再运行构建和部署脚本，最后做上线验证。 |

## 输出风格与失败处理

- 列出执行过的命令、关键参数和摘要输出，例如 `npm run test`、`git push cache main`。
