...
5. **切回主工作树并同步**：
   ...
   ```
6. **Squash 合并后分支处理**：默认结束该功能分支，不继续在原分支上 `rebase`。如需继续开发，基于最新 `origin/$BASE_BRANCH` 新建分支；若必须复用分支，需先明确授权再执行 `git reset --hard origin/$BASE_BRANCH`。
...
- Squash 合并后不得在原功能分支继续 `rebase` 迭代，避免重复提交；应删除或重建分支。
